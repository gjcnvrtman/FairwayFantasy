...
interface Tournament {
  ...
  /** NULL = ESPN hasn't published the field yet; picks page is gated
   *  by `runFieldSync()`. The "Manual Field Upload" section below
   *  lets commissioners seed the field when ESPN is late. */
  field_published_at:      string | null;
}
...
export default function AdminPanel({
  ...
  // ── Manual Field Upload (ESPN-late-publish fallback) ──────────
  // Default-collapsed for the same reason as Pick Deadlines — it's
  // a rare-but-critical action. Auto-opens when there's a tournament
  // in the "field not published, picks are blocked" state so the
  ...
  // ── Manual field-upload handlers ──────────────────────────
  // Per-tournament textarea + button. Used when ESPN doesn't publish
  // the field by the pick deadline (TODO 2026-05-23). Commissioner
  // pastes the player list from the tournament site, server matches
  // each name against the golfers table + seeds scores rows + stamps
  // tournaments.field_published_at. Unmatched names are reported back.
  const [fieldNamesInput, setFieldNamesInput] = useState<Record<string, string>>({});
  ...
  const [fieldUnmatched,  setFieldUnmatched]  = useState<Record<string, string[]>>({});

  async function uploadField(tournamentId: string, namesText: string) {
    setFieldBusy(tournamentId);
    setFieldMsg(prev => ({ ...prev, [tournamentId]: '' }));
    setFieldErr(prev => ({ ...prev, [tournamentId]: '' }));
    setFieldUnmatched(prev => ({ ...prev, [tournamentId]: [] }));
    try {
      const res = await fetch('/api/admin/upload-field', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ slug: league.slug, tournamentId, names: namesText }),
      });
      const data = await res.json().catch(() => ({} as Record<string, unknown>));
      if (Array.isArray(data.unmatched)) {
        setFieldUnmatched(prev => ({ ...prev, [tournamentId]: data.unmatched as string[] }));
      }
      if (!res.ok) {
        const errMsg = typeof data.error === 'string'
          ? data.error
          : `Upload failed (HTTP ${res.status})`;
        setFieldErr(prev => ({ ...prev, [tournamentId]: errMsg }));
        return;
      }
      const matched = typeof data.matched === 'number' ? data.matched : 0;
      const total   = typeof data.totalNames === 'number' ? data.totalNames : 0;
      const unmatched = Array.isArray(data.unmatched) ? data.unmatched.length : 0;
      setFieldMsg(prev => ({
        ...prev,
        [tournamentId]: `Uploaded ${matched}/${total} golfer(s). ` +
          (unmatched > 0
            ? `${unmatched} name(s) unmatched — review below. `
            : '') +
          'Field is published; picks page is now open.',
      }));
      // Clear the textarea on success so a stale paste doesn't get
      // re-uploaded if the commissioner clicks twice.
      setFieldNamesInput(prev => ({ ...prev, [tournamentId]: '' }));
      router.refresh();
    } catch (err) {
      setFieldErr(prev => ({
        ...prev,
  ...
  return (
    <div style={{
      ...
      {/* ── Manual Field Upload (ESPN-late-publish fallback) ─── */}
      {(() => {
        // Show the section ONLY when there's at least one upcoming
        // tournament whose field hasn't been published. Otherwise
        // the section is noise — there's nothing for the commissioner
        // to do here unless ESPN is actually late.
        const dayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const needsField = tournaments
          .filter(t => t.status === 'upcoming')
          .filter(t => t.field_published_at === null)
          .filter(t => new Date(t.start_date) >= dayAgo)
          .sort((a, b) =>
            new Date(a.start_date).getTime() - new Date(b.start_date).getTime(),
          );
        if (needsField.length === 0) return null;
        return (
          <section className="card" aria-labelledby="field-h" style={{ padding: 0, overflow: 'hidden' }}>
            <button
              type="button"
              onClick={() => setFieldOpen(o => !o)}
              aria-expanded={fieldOpen}
              aria-controls="field-body"
              style={{
                display: 'flex',
                width: '100%',
                justifyContent: 'space-between',
                ...
                background: 'transparent',
                border: 'none',
                borderBottom: fieldOpen ? '1px solid var(--cream-dark)' : 'none',
                cursor: 'pointer',
                textAlign: 'left',
              }}
            >
              <h2 id="field-h" style={{
                ...
                <span style={{
                  ...
                  {needsField.length} pending
                </span>
              </h2>
              <span aria-hidden="true" style={{ fontSize: '1.2rem', color: 'var(--slate-mid)' }}>
                {fieldOpen ? '−' : '+'}
              </span>
            </button>

            {fieldOpen && (
            <div id="field-body" style={{ padding: '1.25rem 1.5rem' }}>
              <p style={{ color: 'var(--slate-mid)', fontSize: '0.85rem', marginBottom: '1rem', marginTop: 0 }}>
                ESPN hasn&apos;t published the field for the tournaments below. Picks are
                currently <strong>blocked</strong> for these events. Paste the player list
                (one name per line — CSV exports work too) from the tournament website and
                click <strong>Upload Field</strong> to seed the picks list and unblock picks.
                Unmatched names are reported — re-upload after fixing typos.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {needsField.map(t => {
                  const namesValue = fieldNamesInput[t.id] ?? '';
                  const busy       = fieldBusy === t.id;
                  const msg        = fieldMsg[t.id];
                  const err        = fieldErr[t.id];
                  const unmatched  = fieldUnmatched[t.id] ?? [];
                  return (
                    <div key={t.id} style={{
                      padding: '0.75rem 0.85rem',
                      border: '1px solid var(--cream-dark)',
                      ...
                      <textarea
                        ...
                        value={namesValue}
                        onChange={e => setFieldNamesInput(prev => ({ ...prev, [t.id]: e.target.value }))}
                        disabled={busy}
                        style={{ width: '100%', fontFamily: 'monospace', fontSize: '0.82rem', marginBottom: '0.5rem' }}
                      />
                      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                        <button
                          type="button"
                          className="btn btn-primary btn-sm"
                          disabled={busy || !namesValue.trim()}
                          aria-busy={busy}
                          onClick={() => uploadField(t.id, namesValue)}
                        >
                          {busy ? 'Uploading…' : 'Upload Field'}
                        </button>
                        <button
                          type="button"
                          className="btn btn-outline btn-sm"
                          disabled={busy || !namesValue}
                          onClick={() => setFieldNamesInput(prev => ({ ...prev, [t.id]: '' }))}
                        >
                          Clear
                        </button>
                      </div>
                      {err && (
                        <p className="hint" style={{ color: 'var(--red)', marginTop: '0.5rem' }}>
                          {err}
                        </p>
                      )}
                      {msg && (
                        <p className="hint" style={{ color: 'var(--green-mid)', marginTop: '0.5rem' }}>
                          {msg}
                        </p>
                      )}
                      {unmatched.length > 0 && (
                        <details style={{ marginTop: '0.5rem' }}>
                          <summary style={{ fontSize: '0.8rem', color: 'var(--brass)', cursor: 'pointer' }}>
                            {unmatched.length} unmatched name(s) — click to expand
                          </summary>
                          <ul style={{
                          ...
                          <p style={{ marginTop: '0.4rem', fontSize: '0.75rem', color: 'var(--slate-light)' }}>
                            These names didn&apos;t match any golfer in the database. Check spelling
                            against the golfers page, or seed missing golfers via the rankings sync.
                          </p>
                        </details>
                      )}
                    </div>
