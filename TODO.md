...
- [ ] **`leagues` schema drift — `start_date`, `end_date`, `weekly_bet_amount` columns referenced in code but not created by any SQL** — surfaced 2026-05-23 while wiring the field-publish email notifier. `src/lib/db/schema.ts:31-50` declares all three columns on `LeaguesTable`, and **9 source files** reference them (sync.ts, schedule/stats/history/admin pages, money.ts, admin league-settings + league-delete routes, the league dashboard). But neither `infra/postgres/init/00-schema.sql:28-36` (canonical init script) nor `supabase/schema.sql:12-20` (historical reference) creates them, and `scripts/migrations/00{1..7}-*.sql` never touch the leagues table. Local `\d leagues` confirms only 7 columns: id, name, slug, invite_code, commissioner_id, max_players, created_at. **Implications:** (a) `npm run dev` against a fresh `00-schema.sql` install crashes the schedule page + money math + admin league-settings the moment they read a non-existent column; (b) prod on `.150` clearly works, so DDL must have been applied directly via psql at some point without being captured as a migration; (c) my new field-publish notifier (sync.ts `notifyFieldPublished`) deliberately works around the gap by NOT filtering members by league date window — broader audience than ideal. **Fix:** add `scripts/migrations/008-leagues-date-window-and-bet.sql` that does `ALTER TABLE leagues ADD COLUMN start_date TIMESTAMPTZ, ADD COLUMN end_date TIMESTAMPTZ, ADD COLUMN weekly_bet_amount NUMERIC(10,2) NOT NULL DEFAULT 0;` (defaults so existing prod rows don't fail NOT NULL), AND mirror the same in `infra/postgres/init/00-schema.sql` so fresh installs match. Then restore the league-window filter in `notifyFieldPublished` (drops the comment block that explains the workaround). After the migration lands, audit the 9 call sites to confirm everything assumes non-null dates correctly (the schema.ts types say `Timestamp | null`, so most sites should already handle the null case).
