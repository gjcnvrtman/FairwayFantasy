...
_(`leagues` schema drift closed 2026-05-30 — commit `65c9213`. Migration 008 added `start_date TIMESTAMPTZ`, `end_date TIMESTAMPTZ`, `weekly_bet_amount NUMERIC(10,2) NOT NULL DEFAULT 10.00` (matched the prod-applied DDL's default of 10.00, not the originally-proposed 0). 00-schema.sql leagues block mirrors the inline columns so fresh installs match. `notifyFieldPublished` in `src/lib/sync.ts` had its league-window overlap filter restored (was deliberately broader while columns were drifted). Applied on prod via `psql -f 008-...sql` (no-op — all 3 columns already existed). tsc clean, 269/269 vitest pass, build succeeded, service restarted, site responds 200. See Done.)_
...
- [x] **No demo route originally; resolved in P3** ✓ — addressed during P3 work. Checkbox state corrected 2026-05-19.
...
### Adopt ESPN per-competitor `/status` endpoint as authoritative per-golfer state (2026-07-18)
- [ ] **Motivation** — the whole cut-inference block in `syncTournament` exists to work around `/pga/scoreboard` not exposing per-golfer status, `cutLine`, or R3 tee times. The ESPN core API DOES expose all three at `sports.core.api.espn.com/v2/sports/golf/leagues/pga/events/{eventId}/competitions/{eventId}/competitors/{golferId}/status`. Sample response for Min Woo Lee at The Open R3:
  ```json
  {
    "period": 3, "type": {"name": "STATUS_FINISH", "state": "post", "completed": true},
    "teeTime": "2026-07-18T08:20Z", "hole": 18, "startHole": 1, "thru": 18,
    "position": {"id": "66", "displayName": "T66", "isTie": true}, "playoff": false
  }
  ```
- [ ] **Cost** — one HTTP request per golfer per sync cycle (~156 requests for a full major field). Every 10 min that's ~15 req/sec sustained across a tournament weekend. Batch with `Promise.all` and a small concurrency limit (say 8) to keep ESPN happy.
- [ ] **Wins if adopted** — (a) eliminates `inferCutRule` + `applyCutRule` fallback path entirely (ESPN's `type.name` is authoritative — no more `topN`+ties heuristics), (b) exposes `teeTime` for R3/R4 pairings UI, (c) gives us `position` string ("T66") directly rather than deriving from order, (d) closes the currently-open unit-consistency bug where `cut_score` sometimes holds strokes, sometimes to-par (see below).
- [ ] **Latent bug this same audit surfaced** — `tournaments.cut_score` is stored in stroke totals when ESPN's `situation.cutLine.value` is present (see `fetchLiveLeaderboard` in `src/lib/espn.ts`) but in score-to-par when we infer via `applyCutRule` on normalized scoreboard linescores. Same column, two units. This year the stroke-vs-to-par mismatch was silently benign because stroke totals are always >> to-par sums so nobody got flagged when the ESPN cutLine path fired. If a tournament ever lands on the OTHER path where a stroke-total cut is stored and then a to-par sum is compared against it (e.g. resync after switch of endpoints), everyone would be under. Fix independently of #4 above or as part of it.
...
- [x] **`isReplacementEligible` vs in-route inline check reconciled** *(P1 #5.6)* — Shipped 2026-05-17 in `65717f6`. Helper rewritten to take the actual `scores`-row shape (`status` + `round_1`, no synthetic `teed_off`); route now selects both columns and delegates. Closes a real correctness gap: pre-tournament WD or pre-DQ golfers (status≠'active' but round_1 IS NULL) would have passed the inline-only check and become legal replacements. 7 new tests pin the contract (207/207 overall).
...
- [x] **Delete-league capability** — Shipped 2026-05-19 in `c8ef20b`. New `POST /api/admin/league-delete` gated by `requireCommissioner(slug)` with body `{ slug, confirmName }`; server re-verifies the typed name matches `league.name` exactly. Refuses with 409 if any tournament currently in `status='active'` or `cut_made` overlaps the league window. Schema's existing `ON DELETE CASCADE` on every `league_id` FK handles dependent rows. Audit trail: logs per-user money totals to the server log at delete time so members have a paper trail. UI: AdminPanel gains a collapsed-by-default "⚠ Danger Zone" card at the bottom — expand to reveal a "type the league name to confirm" input + Delete button (disabled until match). On success `router.push('/dashboard')`. ✓

---

## P3 — research / tuning / docs

_(`NEXTAUTH_URL` on prod closed 2026-05-30 — verified already set to `https://fairway.golf-czar.com` on `.150`. The original 2026-05-20 entry described a value of `http://192.168.1.160:3000`, but by the time today's check ran the value was already correct (likely fixed in an earlier ad-hoc session and never closed in TODO). sed was a no-op; backup file `.env.local.bak.20260530-fix` left in place as audit trail. Service restarted clean, still active.)_
...
- [x] **Migration 008 — `leagues.start_date` / `end_date` / `weekly_bet_amount`** — commit `65c9213`. Original 5/23 TODO entry: three columns referenced from 9 source files (sync.ts, schedule/stats/history/admin pages, money.ts, league admin routes) but never created by any committed SQL. Prod had them via hand-applied psql DDL never captured. `npm run dev` against a fresh `00-schema.sql` install would have crashed schedule/money/admin paths.
  * [scripts/migrations/008-leagues-date-window-and-bet.sql](scripts/migrations/008-leagues-date-window-and-bet.sql): `ADD COLUMN IF NOT EXISTS` for all three. `weekly_bet_amount` default 10.00 matches the prod-applied DDL (not the 0 the original entry suggested). Verify block via `RAISE EXCEPTION` mirrors migration 001's pattern.
  * [infra/postgres/init/00-schema.sql](infra/postgres/init/00-schema.sql): leagues table inline now includes all three columns so fresh installs match.
  * [src/lib/sync.ts](src/lib/sync.ts) `notifyFieldPublished`: restored the league-window overlap filter that was deliberately disabled while the columns were drifted. NULL on either side = no constraint (open-ended leagues always receive). Tournament select pulls `start_date`/`end_date` for the comparison.
  * **Verified:** tsc clean, 269/269 vitest pass, build succeeded, migration applied on prod (all 3 ADD COLUMNs were NOTICE: already exists; COMMENTs landed; verify DO-block confirmed all 3 present), service restarted clean, `curl https://fairway.golf-czar.com` returns HTTP 200.

- [x] **`NEXTAUTH_URL` on prod — already fixed, TODO entry was stale.** The 5/20 entry described a value of `http://192.168.1.160:3000` but today's check found `https://fairway.golf-czar.com` already in place on `.150`. Almost certainly fixed in an ad-hoc session and never closed in TODO. Today's sed was a no-op; backup `.env.local.bak.20260530-fix` left as audit trail. Service restarted clean.
...
- **P0 status-flip** (`src/lib/sync.ts`): `syncTournament` now infers tournament completion from the linescores array. ESPN's `/pga/scoreboard` fallback (which every recent major uses) never reports `STATUS_IN_PROGRESS → STATUS_FINAL`, so the old `status.toLowerCase().includes('final')` branch never fired and tournaments stayed in `cut_made` until Monday 06:00's weekly maintenance sweep — leaving the money card empty for ~14 hours every Sunday night. Fix: after the cut-inference block, compute `completionByLinescore`: `true` when `tournament.end_date < now` AND every cut survivor (linescores.length >= 3) has `linescores.length === 4` (un-played future rounds are filtered out of the linescores array by `normalizeScoreboardCompetitor`, so length 4 = all four rounds played). Combined with the existing 'final' substring check. Edge cases handled by the design: mid-Sunday → `every` fails, stays cut_made; no cut survivors → `survivors.length > 0` guard prevents flip; weather-shortened 54-hole tournaments → never matches length 4, falls through to Monday maintenance sweep (which already force-flips on `end_date < now-24h`).

- **P2 recomputeResults batching** (`src/lib/sync.ts`): `fantasy_results` and `season_standings` upserts now use a single batched `INSERT … ON CONFLICT` each instead of one statement per row. Old code: nested loops with one auto-commit (fsync) per row, ~25 commits per sync cycle at 5 leagues × 5 members. New code: 2 statements total. Same semantics — identical conflict columns, doUpdateSet, and `updated_at` (shared across both tables now so they reflect a single cycle). 259/259 tests still pass; no API surface change.
