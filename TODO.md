...
- [ ] **ESPN tournament status never flips to `complete` — money math is therefore frozen behind it** — Surfaced 2026-05-17 with the per-tournament money feature. `sync.ts:syncTournament` sets `newStatus = 'complete'` only when ESPN's leaderboard `status.toLowerCase().includes('final')`. The /pga/scoreboard fallback (which is what every recent major uses, since /pga/leaderboard is 404) returns `STATUS_IN_PROGRESS` even after the trophy ceremony — and stays there indefinitely. PGA Championship is a current example: post-Round-4 Sunday with status still `active`/`cut_made`. Effect: `computeLeagueMoney` filters `getCompletedTournamentsInRange` to `status='complete'`, so a tournament whose money should have been settled never gets there. Money card stays empty even after the event ends. Two fixes to consider together: **(a) time-based completion** — if `tournament.end_date < NOW() - X hours` AND every cut-survivor has a non-null `round_4`, treat as complete (similar pattern to the `cutHasBeenMade ⇐ currentRound >= 3` heuristic we already use). **(b) maintenance sweep** — once a week (already on `fairway-rankings.timer`) scan stuck-active rows whose end_date is well in the past and force-flip them to complete. Either fix is ~30 min in `src/lib/sync.ts`. Until landed, the workaround is a commissioner manually running `UPDATE tournaments SET status='complete' WHERE id='...'` once the trophy's handed out.
...
- [ ] **Delete-league capability** — Surfaced 2026-05-17. There's no way to delete a league from the UI today. The schema already cascades on `league_id` (`ON DELETE CASCADE` on `league_members`, `picks`, `fantasy_results`, `season_standings`, `reminder_log`), so a single `DELETE FROM leagues WHERE id=…` would remove everything. What's missing is the commissioner-side flow: (a) a new `DELETE /api/leagues` or `POST /api/admin/league-delete` endpoint gated by `requireCommissioner`; (b) a confirmation step in the Admin panel — destructive enough that "type the league name to confirm" is the right friction; (c) a redirect target (probably `/dashboard`) once the row is gone. Edge cases: don't delete while ANY tournament is currently active (the score-sync timer would keep trying to recompute results for a league that no longer exists — actually it just won't find rows and no-ops, but worth a guard); decide whether to allow delete when money is "owed" between members (currently no settlement system, so probably yes — but log the per-user totals at delete time so they have a record). ~1-2 hours including UI + commit hook + a couple of tests.
